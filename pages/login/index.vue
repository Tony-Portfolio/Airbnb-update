<template>
    <div>
        <main class="w-full h-full">
            <section class="w-full h-full">
                <div class="flex items-center justify-center w-full h-full text-black h-screen">
                    <div
                        class="my-auto max-w-[500px] w-full block mx-auto p-8 bg-cover bg-center sm:text-black relative border-[1px] border-black/[0.1] rounded-lg shadow-md">
                        <h2 class="uppercase text-center font-bold text-2xl">Masuk ke akun Anda</h2>
                        <p class="sm:text-black/[0.8] text-sm text-center">Belum punya akun? <NuxtLink
                                to="/register" class="font-bold text-[#FF385C] bg-white rounded p-[2px]">Daftar disini
                            </NuxtLink>
                        </p>
                        <NuxtLink to="/"><img src="/images/airbnb-logo.png" alt=""
                            class="w-7/12 block m-auto my-10 sm:bg-transparent sm:rounded-none"></NuxtLink>
                        <div class="flex flex-col justify-center gap-4">
                            <div class="">
                                <label for="username" class="text-left font-[500] text-sm mb-2 block w-fit">Username</label>
                                <input type="text" placeholder="Username"
                                    class="border-2 border-black-800/[0.1] p-3 px-4 w-full rounded text-sm font-bold text-black"
                                    id="username" name="username" v-model="username">
                            </div>
                            <div class="">
                                <!-- <input type="text" placeholder="Email" class = "border-2 border-slate-800/[0.2] p-3 px-4 w-full rounded text-sm"> -->
                                <label for="password" class="text-left font-[500] text-sm mb-2 block w-fit">Password</label>
                                <input type="password" placeholder="Password"
                                    class="border-2 border-black-800/[0.1] p-3 px-4 w-full rounded text-sm font-bold text-black"
                                    id="password" name="password" v-model="password">
                            </div>
                            <button class="w-full bg-[#FF385C] p-2 rounded text-white outline-none" name="submit"
                                @click="getDataUser">Masuk</button>
                        </div>
                    </div>
                </div>
            </section>
        </main>

    </div>
</template>

<script>

import axios from 'axios';


export default {
    data() {
        return {
            layouts: {
                header: "header",
                container: "container",
                footer: "footer"
            },
            username: "",
            password: "",
            userData: [],
        }
    },
    computed: {
    },
    methods: {
        getDataUser() {
            const username = this.username;
            const password = this.password;
            const jsonUserData = JSON.stringify(this.userData);
            const getUserData = JSON.parse(localStorage.getItem(username));
            if (getUserData) {
                if (getUserData[0].password == password) {
                    var isLogged = true
                    const loginData = [{isloggedin:true,username}]
                    localStorage.setItem("login", JSON.stringify(loginData));
                    window.location.href = "/"
                }else{
                    console.log(password, getUserData)
                    alert('username atau password salah!')
                }
            } else {
                alert('username tidak ada!')
            }
        }
    },
    async created() {
    },
}
</script>